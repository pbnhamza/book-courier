...
const Register = () => {
  ...
  const validatePassword = (pwd) => {
    const errors = [];
    if (!passwordRegex.minLength.test(pwd)) errors.push('At least 6 characters');
    if (!passwordRegex.uppercase.test(pwd)) errors.push('One uppercase letter');
    if (!passwordRegex.lowercase.test(pwd)) errors.push('One lowercase letter');
    if (!passwordRegex.number.test(pwd)) errors.push('One number');
    if (!passwordRegex.special.test(pwd)) errors.push('One special character');
    return errors;
  };
  ...
  const handleRegister = async (event) => {
    ...
    if (!regExp.test(password)) {
      toast.error('Invalid password! Password must have at least 6 characters, including uppercase, lowercase, a number, and a special character.');
      setError('Invalid password! Password must have at least 6 characters, including uppercase, lowercase, a number, and a special character.');
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      toast.success('Registration successful! Welcome to BookCourier!');
      navigate('/');
    } catch (error) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-linear-to-br from-indigo-50 via-purple-50 to-pink-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900 relative overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-md bg-base-100 shadow-2xl rounded-2xl p-8">
        ...
        <div className="mb-6 text-center">
          ...
          <p className="text-base-content/60">Join us and start your reading journey</p>
        </div>
        ...
        <form onSubmit={handleRegister} className="space-y-4">
          ...
          <div className="form-control">
            ...
            <div className="relative group">
              <div className="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none">
                <div className="w-10 h-10 flex items-center justify-center rounded-lg bg-linear-to-br from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20 group-focus-within:from-indigo-100 group-focus-within:to-purple-100 dark:group-focus-within:from-indigo-900/40 dark:group-focus-within:to-purple-900/40 transition-all">
          ...
          <div className="form-control">
            <label className="label pb-2">
              <span className="label-text font-semibold text-base">Photo URL (Optional)</span>
            </label>
            <div className="relative group">
              <div className="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none">
                <div className="w-10 h-10 flex items-center justify-center rounded-lg bg-linear-to-br from-indigo-50 to-purple-50 dark:from-indigo-900/20 dark:to-purple-900/20 group-focus-within:from-indigo-100 group-focus-within:to-purple-100 dark:group-focus-within:from-indigo-900/40 dark:group-focus-within:to-purple-900/40 transition-all">
            ...
            {password && (
              <div className="mt-3 space-y-2">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-semibold text-base-content/70">Password Strength</span>
                  <span className={`text-xs font-bold ${
                    passwordErrors.length === 0 ? 'text-green-600' : 
                    passwordErrors.length <= 2 ? 'text-yellow-600' : 'text-red-600'
                  }`}>
                ...
                <div className="space-y-1.5 mt-3">
                  {[
                    { text: 'At least 6 characters', check: passwordRegex.minLength.test(password) },
                    { text: 'One uppercase letter (A-Z)', check: passwordRegex.uppercase.test(password) },
                    { text: 'One lowercase letter (a-z)', check: passwordRegex.lowercase.test(password) },
                    { text: 'One number (0-9)', check: passwordRegex.number.test(password) },
                    { text: 'One special character (!@#$...)', check: passwordRegex.special.test(password) }
                    ...
                    <div key={index} className="flex items-center gap-2">
                      ...
                      <span className={`text-xs ${requirement.check ? 'text-green-600 font-medium' : 'text-base-content/60'}`}>
                        {requirement.text}
                      </span>
                    </div>
                  ))}
          ...
          <button
            type="submit"
            disabled={loading}
            className="btn w-full h-14 text-base bg-linear-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white border-none rounded-xl shadow-lg hover:shadow-xl hover:scale-[1.02] active:scale-[0.98] transition-all duration-200 font-semibold mt-2"
          >
            {loading ? (
          ...
          {/* Divider */}
          <div className="divider text-sm text-base-content/50">Or register with</div>
